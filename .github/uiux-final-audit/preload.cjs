...
const assets = names.map((name, index) => ({
  assetId: `asset-${index + 1}`, locationId: `location-${index + 1}`, displayName: name,
  ...
  byteSize: 720000 + index * 183000, category: categories[index % categories.length], previewKind: index === 22 ? 'none' : 'image',
...
const detailFor = (asset) => ({
  assetId: asset.assetId, locationId: asset.locationId, originalDisplayName: asset.displayName, relativeDisplayPath: asset.relativeDisplayPath,
  mediaFamily: asset.mediaFamily, mimeType: asset.mimeType, extension: asset.extension, byteSize: asset.byteSize, category: asset.category,
  previewKind: asset.previewKind, availability: asset.availability, reviewState: asset.reviewState, customTitle: asset.customTitle,
  ...
  tags: asset.tags, usedIn: asset.usedIn, revision: asset.revision, collectionIds: asset.assetId === 'asset-1' ? ['collection-1','collection-2'] : [],
});
const facetCounts = (items, getter) => [...items.reduce((map, item) => { for (const value of getter(item)) map.set(value,(map.get(value)||0)+1); return map; }, new Map())]
  .map(([value,count]) => ({ value, count })).sort((a,b) => b.count-a.count || a.value.localeCompare(b.value));
const facetsFor = (items) => ({ categories: facetCounts(items,a=>[a.category]), extensions: facetCounts(items,a=>[a.extension].filter(Boolean)), mediaFamilies: facetCounts(items,a=>[a.mediaFamily]), tags: facetCounts(items,a=>a.tags), usedIn: facetCounts(items,a=>a.usedIn) });
...
const filterAssets = (query) => {
  let items = [...assets]; const includes=(selected,values)=>!selected.length||selected.every(value=>values.includes(value));
  ...
  const sorters={name_ascending:(a,b)=>a.displayName.localeCompare(b.displayName),name_descending:(a,b)=>b.displayName.localeCompare(a.displayName),created_ascending:(a,b)=>a.createdAtMs-b.createdAtMs,created_descending:(a,b)=>b.createdAtMs-a.createdAtMs,size_ascending:(a,b)=>a.byteSize-b.byteSize,size_descending:(a,b)=>b.byteSize-a.byteSize,review_state:(a,b)=>a.reviewState.localeCompare(b.reviewState)};
  return items.sort(sorters[query.sort]||sorters.created_ascending);
};
const svgFor = (assetId) => {
  const index=Math.max(0,Number(assetId.split('-')[1]||1)-1),a=palette[index%palette.length],b=palette[(index+4)%palette.length];
...
contextBridge.exposeInMainWorld('referenceLibrary', {
  version:4, createLibrary:async()=>session, openLibrary:async()=>session, completeOpenIntent:async()=>session,
  readPreferences:async()=>({...preferences}), writePreferences:async patch=>({...Object.assign(preferences,patch)}), closeLibrary:async()=>{}, chooseRoot:async()=>null,
  listRoots:async()=>roots.map(root=>({...root})), reauthorizeRoot:async()=>({session,root:{...roots[1],authorized:true,state:'ready'}}), scanRoot:async(_s,rootId)=>({rootId,jobId:'job-1'}), cancelJob:async()=>{},
  ...
  getAsset:async(_s,id)=>detailFor(assets.find(a=>a.assetId===id)), updateAsset:async input=>{const a=assets.find(a=>a.assetId===input.assetId);if(input.patch.reviewState)a.reviewState=input.patch.reviewState;a.revision++;return{asset:detailFor(a),libraryRevision:2};},
  listCollections:async()=>collections.map(collection=>({...collection})), createCollection:async(_s,name)=>({collectionId:'new',name,assetCount:0,revision:1}), renameCollection:async(_s,id,_r,name)=>({collectionId:id,name,assetCount:0,revision:2}), deleteCollection:async()=>{},
  setCollectionMembership:async({collectionId,assetIds})=>({collectionId,affected:assetIds.length,libraryRevision:2}), assetResourceUrl:({assetId})=>svgFor(assetId),
  revealLocation:async()=>{}, openLocation:async()=>{}, copyLocationPath:async()=>{}, queryCapabilities:async()=>[], restartCore:async()=>session, subscribe:()=>()=>{},
});
