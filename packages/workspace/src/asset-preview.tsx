  ...
  return (
    <div className="preview" role="dialog" aria-modal="true" aria-label={`Preview ${props.asset.displayName}`} onKeyDown={(event) => handleDialogKey(event, props.onClose)}>
      ...
      {failed || unavailable ? (
        ...
        <div ref={stage} className={fit ? "preview__stage preview__stage--fit" : "preview__stage"}>
          <img alt={props.asset.displayName} src={props.source} style={fit ? undefined : { width: `${zoom * 100}%` }} onError={() => setFailed(true)} />
        </div>
      )}
    </div>
  );
